...
# Download GitHub CLI
gh_token_url=$(resolve_github_release_asset_url "Link-/gh-token" "contains(\"linux\") and endswith(\"${arch}\") "latest")

gh_token_path=$(download_with_retry "$gh_token_url")

sudo mv "$gh_token_path" /usr/local/bin/gh-token
sudo chmod +x /usr/local/bin/gh-token
